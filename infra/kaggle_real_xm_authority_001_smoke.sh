...
# smoke scheduling plus run-custody isolation so the output belongs to one run.
...
# Custody isolation: previous Kaggle attempts used the same observer JSONL path,
# so stale records could append into a later smoke summary. Clear only the
# infrastructure smoke root before launching. The frozen repository checkout is
# re-cloned by the base launcher immediately afterward.
rm -rf "$ROOT"
