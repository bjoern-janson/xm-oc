...
# REAL-XM-AUTHORITY-001 fresh-seed replication of high-K authority/competence coupling.
# Scientific code remains detached at FROZEN_SHA. Execution helpers/protocol are
...
# Clean custody: one invocation is one complete replication campaign.
rm -rf "$ROOT"
mkdir -p "$RUNNER_DIR" "$CACHE_DIR" "$OUT_DIR"

echo "=== REAL-XM-AUTHORITY-001 HIGH-K COUPLING REPLICATION ==="
...
echo "PRIMARY: seed-block Delta=Z_late(K12)-Z_late(K2), Z_late averages Fisher-z at steps 1536 and 2048."
echo "CLAIM CEILING: replicate authority/competence association only; no causal direction; no Q_gen claim."
...
# Exact frozen scientific organism.
...
# Runner helpers live outside the checkout; expose the frozen repo root without editing it.
...
# Immutable replication helpers/protocol.
...
sha256sum \
  ...
  "$RUNNER_DIR/run_seeded.py" \
  "$RUNNER_DIR/summarize_replication.py" \
  "$RUNNER_DIR/REAL_XM_AUTHORITY_001_HIGH_K_REPLICATION.md" \
  | tee "$OUT_DIR/execution-bundle-files.sha256"

# Rebuild the exact previously recorded ImageNet specimen. The pinned cache builder
# fails closed on custody-hash mismatch.
...
for SEED in "${SEEDS[@]}"; do
  ...
  for K in "${K_VALUES[@]}"; do
    ...
    mkdir -p "$OBS_DIR" "$MEMBER/checkpoints"
    ...
    git -C "$REPO_DIR" diff --quiet && git -C "$REPO_DIR" diff --cached --quiet || {
      echo "FATAL: scientific checkout dirty before seed=$SEED K=$K" >&2
      exit 100
    }
    ...
    python "$RUNNER_DIR/run_seeded.py" \
      ...
      --save_top_k_ckpts 0 \
      --checkpoint_base_dir "$MEMBER/checkpoints" \
      --wandb_project "real-xm-authority-001-high-k-replication" \
      --wandb_offline \
    ...
    if [[ "$TRAIN_RC" -ne 0 ]]; then
      echo "FATAL: seed=$SEED K=$K training failed with exit code $TRAIN_RC" >&2
      exit "$TRAIN_RC"
    fi

    git -C "$REPO_DIR" diff --quiet && git -C "$REPO_DIR" diff --cached --quiet || {
      echo "FATAL: scientific checkout changed during seed=$SEED K=$K" >&2
      exit 101
    }

    grep -Fq "REPLICATION_SEED_INJECTION $SEED" "$MEMBER/train.log" || {
      echo "FATAL: missing external seed injection audit seed=$SEED K=$K" >&2
      exit 102
    }
    grep -Fq "Seed set to $SEED" "$MEMBER/train.log" || {
      echo "FATAL: frozen trainer did not report requested seed=$SEED K=$K" >&2
      exit 103
    }
...
ckpts = sorted(Path("$MEMBER/checkpoints").rglob("last.ckpt"))
print("last_ckpts", [str(p) for p in ckpts])
assert len(ckpts) == 1
print("REPLICATION_MEMBER_AUDIT_PASS", seed, K)
PY

    CKPT="$(find "$MEMBER/checkpoints" -name last.ckpt -type f -print -quit)"
    sha256sum \
...
python "$RUNNER_DIR/summarize_replication.py" \
  --root "$ROOT" \
  --output "$OUT_DIR/replication-summary.json" \
  --frozen-sha "$FROZEN_SHA" \
  --execution-bundle-sha "$EXECUTION_BUNDLE_SHA" \
  2>&1 | tee "$OUT_DIR/replication-summary.txt"

# Custody over campaign-level artifacts and all member custody ledgers.
...
sha256sum \
  ...
  "$OUT_DIR/replication-summary.json" \
  "$OUT_DIR/replication-summary.txt" \
  "$OUT_DIR/environment-audit.txt" \
  "$OUT_DIR/execution-bundle-files.sha256" \
  "$OUT_DIR/member-custody-ledgers.sha256" \
  | tee "$OUT_DIR/replication-custody.sha256"
