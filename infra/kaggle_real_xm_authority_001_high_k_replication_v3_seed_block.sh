...
# REAL-XM-AUTHORITY-001 v3 infrastructure-only segmented execution.
# Runs exactly one preregistered training-seed block K={2,8,12}, audits it,
# and packages the block immediately so no single Kaggle runtime must survive
# the complete five-seed campaign.
...
ALLOWED_SEEDS=(101 202 303 404 505)
K_VALUES=(2 8 12)
BATCH_SIZE=8
MAX_STEPS=2049
WARMUP_STEPS=20
...
SEED="${1:-}"
[[ -n "$SEED" ]] || { echo "usage: $0 <101|202|303|404|505>" >&2; exit 2; }
case " $SEED " in
  " 101 "|" 202 "|" 303 "|" 404 "|" 505 ") ;;
  *) echo "FATAL: seed must be one of ${ALLOWED_SEEDS[*]}" >&2; exit 3 ;;
esac

ROOT="/kaggle/working/real_xm_authority_001_high_k_repl_seed${SEED}"
REPO_DIR="$ROOT/xm-oc"
RUNNER_DIR="$ROOT/runner"
CACHE_DIR="$ROOT/cached_latents/imagenet"
OUT_DIR="$ROOT/output"
BLOCK_DIR="$ROOT/seed${SEED}"
ARCHIVE="/kaggle/working/real_xm_authority_001_seed${SEED}_block.tar.gz"

rm -rf "$ROOT"
rm -f "$ARCHIVE"
mkdir -p "$RUNNER_DIR" "$CACHE_DIR" "$OUT_DIR"

echo "=== REAL-XM-AUTHORITY-001 HIGH-K REPLICATION V3 SEED BLOCK ==="
...
echo "CLAIM CEILING: this command records one replication unit only; it does not summarize or open the five-seed endpoint."
...
python "$RUNNER_DIR/build_cache.py" \
  --cache-dir "$CACHE_DIR" \
  --manifest "$OUT_DIR/replication-data-manifest.json" \
...
for K in "${K_VALUES[@]}"; do
  ...
  grep -Fq "REPLICATION_SEED_INJECTION $SEED" "$MEMBER/train.log" || { echo "FATAL: missing seed injection audit" >&2; exit 102; }
  grep -Fq "Seed set to $SEED" "$MEMBER/train.log" || { echo "FATAL: frozen trainer seed mismatch" >&2; exit 103; }
  grep -Fq "REPLICATION_CHECKPOINT_WRITES_DISABLED" "$MEMBER/train.log" || { echo "FATAL: checkpoint suppression audit missing" >&2; exit 104; }
...
echo "=== SEED BLOCK $SEED AUDITED ==="

python - <<PY > "$OUT_DIR/block-metadata.json"
import hashlib, json, pathlib
root=pathlib.Path("$ROOT")
out=root/"output"
manifest=json.loads((out/"replication-data-manifest.json").read_text())
meta={
  "kind":"REAL-XM-AUTHORITY-001-high-k-replication-v3-seed-block",
  "seed":int($SEED),
  "k_values":[2,8,12],
  "frozen_sha":"$FROZEN_SHA",
  "execution_bundle_sha":"$EXECUTION_BUNDLE_SHA",
  "dataset":"$HF_DATASET",
  "dataset_revision":"$HF_DATASET_REV",
...
sha256sum \
  "$OUT_DIR/replication-data-manifest.json" \
  "$OUT_DIR/block-metadata.json" \
  "$OUT_DIR/environment-audit.txt" \
  "$OUT_DIR/execution-bundle-files.sha256" \
  "$BLOCK_DIR/k2/custody.sha256" \
  "$BLOCK_DIR/k8/custody.sha256" \
  "$BLOCK_DIR/k12/custody.sha256" \
  | tee "$OUT_DIR/block-custody.sha256"

# Package only measurement/custody artifacts needed for later assembly; omit repo,
# cached latents, HF cache, and W&B working state.
STAGE="$ROOT/archive_stage"
mkdir -p "$STAGE"
cp -a "$BLOCK_DIR" "$STAGE/"
mkdir -p "$STAGE/output"
cp "$OUT_DIR/replication-data-manifest.json" "$STAGE/output/"
cp "$OUT_DIR/block-metadata.json" "$STAGE/output/"
cp "$OUT_DIR/environment-audit.txt" "$STAGE/output/"
cp "$OUT_DIR/execution-bundle-files.sha256" "$STAGE/output/"
cp "$OUT_DIR/block-custody.sha256" "$STAGE/output/"

tar -C "$STAGE" -czf "$ARCHIVE" .
sha256sum "$ARCHIVE" | tee "$OUT_DIR/block-archive.sha256"

echo "REAL_DATA_HIGH_K_COUPLING_REPLICATION_SEED_BLOCK_RECORDED $SEED"
echo "ARCHIVE $ARCHIVE"
echo "IMPORTANT: preserve/download this archive before ending the Kaggle session."
echo "Do not inspect or summarize the five-seed endpoint until all five frozen blocks are assembled."
